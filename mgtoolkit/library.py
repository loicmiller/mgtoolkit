...
class Metagraph(object):
    ...
    def adjacency_matrix(self):
        ...
        triples_lookup=dict()
        for edge in self.edges:
            for elt1 in sorted(edge.invertex):
                for elt2 in sorted(edge.outvertex):
    ...
    def get_projection(self, generator_subset):
        ...
        for index, triples_list in list(triples_to_merge.items()):
...
class ConditionalMetagraph(Metagraph):
    ...
    def get_all_metapaths(self):
        ...
        from itertools import combinations
        all_subsets=sum([list(combinations(self.nodes, r)) for r in range(1, len(self.nodes)+1)], [])
    ...
    def multiply_triple_lists(self, triple_list1, triple_list2, x_i, x_j, x_k):
        ...
        if len(triples_list)>0:
            ...
            for triple in triples_list:
                if triple not in unique_triples_list:
                    unique_triples_list.append(triple)
            return unique_triples_list
        ...
        try:
            ...
            for edge in edge_list:
                ...
                if len(list(edge.invertex))>1:
                    # is a cluster
                    if edge.invertex not in list(clusters.values()):
                        # create new
                        ...
                        # use existing
                        inv = list(clusters.values()).index(edge.invertex)
                ...
                if len(list(edge.outvertex))>1:
                    # a cluster
                    if edge.outvertex not in list(clusters.values()):
                        # create new
                        ...
                        # use existing
                        outv = list(clusters.values()).index(edge.outvertex)
            ...
            # clusters first
            for index, content in list(clusters.items()):
